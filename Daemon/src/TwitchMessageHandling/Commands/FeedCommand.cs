    ...
    internal class FeedCommand() : Command("feed", PermissionLevel.Viewer) {
        internal override async Task ExecuteAsync(
            ...
            if (Random.Shared.Next(secondsSinceLastUse) == 0) {
                feedData.Count = 0;
                await twitchClient.SendReplyAsync(args.ChatMessage.Channel, args.ChatMessage.Id, "popCat BARF2 BARF3 you fed the cat too fast!");
            } else {
                ++feedData.Count;
                if (feedData.Count > feedData.RecordCount) {
                    feedData.RecordCount = feedData.Count;
                    feedData.RecordHolder = args.ChatMessage.DisplayName;
                    feedData.DateRecordSet = DateTime.UtcNow;
                }

                await twitchClient.SendReplyAsync(args.ChatMessage.Channel, args.ChatMessage.Id, $"popCat CrayonTime The cat has been fed {feedData.Count} time{(feedData.Count == 1 ? "" : "s")} in a row");
            }
